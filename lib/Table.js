            ...
            if (item1Type === 'string') {
                ...
                item1.forEach(item2 => {
                    if (typeof item2 === 'string') {
                        if (!fieldsSet.has(item2)) {
                            throw new Error(`invalid field '${item2}'.`);
                        }
                        fields.push({
                            field: item2,
                            descending: false,
                ...
                if (!('fields' in item1)) {
                    throw new Error(`invalid fields '${item1}'`);
                }
                const index = {
                    fields: [],
                    unique: item1.unique ? !!item1.unique : false,
                };
                const fields = item1.fields;
                if (typeof fields === 'string') {
                    ...
                    fields.forEach(item2 => {
                        ...
                        if (item2Type === 'string') {
                            ...
                            index.fields.push({
                                field: item2,
                                descending: item2.descending ? !!item2.descending : false,
                            });
