...
class Table {
    ...
    static _getUniqueIndexes(arr, fieldsSet, uk, uks) {
        ...
        return {...uniqueIndexes, ...this._getIndexes(arr, fieldsSet)};
    }
    ...
    constructor(db, name, fieldTypes, {
        ...
        for (const field in fieldTypes) {
            ...
            if (typeof fieldType === 'string') {
                ...
                if (!fieldType.length) {
                    throw new Error('fieldTypes empty.');
                }
    ...
    getIndexesDef({
        ...
        this.indexes.forEach(index => {
            const fields = [];
            const fieldStmts = [];
            index.fields.forEach(item => {
                const field = item.field;
                fields.push(field);
                fieldStmts.push(`  ${field} ${item.descending ? 'DESC' : 'ASC'}`);
            });
            const uniqueStmt = index.unique ? 'UNIQUE INDEX' : 'INDEX';
            const stmt = `CREATE ${uniqueStmt}${ifNotExistsStmt} "main"."${this.name}_${fields.join('_')}"\nON "${this.name}" (\n` +
                fieldStmts.join(',\n') + '\n);';
            stmts.push(stmt);
        });
        return stmts.join();
    ...
    getTableDef({
        ...
        const ukDef = this.uk.length ? '\n' + this.getCreateUkIndexSql() : '';
        const uksDef = this.uks.length ? '\n' + this.getCreateUksIndexSql() : '';
        return `${tableDef}${ukDef}${uksDef}`;
    }

    getCreateIndexSql(field, {
        unique = false,
        ifNotExists = true,
    } = {}) {
        if (!field.length) {
        ...
        const ifNotExistsStmt = ifNotExists ? ' IF NOT EXISTS' : '';
        return `CREATE ${uniqueStmt}${ifNotExistsStmt} ` +
            `"main"."${this.name}_${field.join('_')}" ` +
            `ON "${this.name}" (${stmts.join(',')});`;
    }

    getCreateUkIndexSql({
        ifNotExists = true,
    } = {}) {
        return this.getCreateIndexSql(this.uk, {unique: true, ifNotExists});
    }

    getCreateUksIndexSql({
         ifNotExists = true,
     } = {}) {
        return this.getCreateIndexSql(this.uks, {unique: true, ifNotExists});
    }
