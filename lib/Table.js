...
class Table {
        ...
        arr.forEach(item1 => {
            ...
            if (item1Type === 'string') {
                ...
                for (const field in item1) {
                    ...
                    index[field] = {
                        descending: Db.getFieldValue(item1[field].descending, 'boolean'),
                    };
                    fields.push(field);
                }
                if (fields.length) {
    ...
    async inserts(objects, {fields = [], chunkSize = 1000} = {}) {
        ...
        if (this._hasT && !(this.t in objects[0])) {
            const t= Db.timestamp();
            for (const object of objects) {
                object[this.t] = t; // add timestamp to object
            }
            if (!(this.t in fields)) {
                fields.push(this.t);
            }
    ...
    async selectUkByPk(pk) {
        return await this.select({pk, resultField: this.uk});
    }

    async selectKByPk(pk) {
        return await this.select({pk, resultField: this.k});
    }
    ...
    async selectPkByUk(uk) {
        return await this.select({uk, resultField: this.pk});
    }

    async selectKByUk(uk) {
        return await this.select({uk, resultField: this.k});
    }
}
