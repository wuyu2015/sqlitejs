...
class Table {
    ...
    getTableDef(customFieldOptions = {}, {
        ifNotExists = true,
    } = {}) {
        const fieldOptions = {};
        this.fieldsWithPk.forEach(field => {
            const option = customFieldOptions[field] ?? {};
            if (field === this.pk) {
                option.pk = true;
            } else {
                delete option.pk;
        ...
        const tableDef = Db.getTableDef(this.name, fieldOptions, {ifNotExists});
