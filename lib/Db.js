...
class Db {
    ...
    static _getWhereStatement(where) {
        ...
        for (const whereArr of where) {
            if (!Array.isArray(whereArr) || whereArr.length < 2) {
                throw new Error(`Invalid whereArr '${whereArr}'.`);
            }
            const op = whereArr[2] ?? '=';
    ...
    static getFieldValue(value, fieldType) {
    ...
    static _getFieldValues(row, fieldTypes) {
        for (const field in fieldTypes) {
            if (field in row) {
                row[field] = Db.getFieldValue(row[field], fieldTypes[field]);
            }
        }
        return row;
    }

    static getSqlType(fieldType) {
        switch (fieldType) {
            case 'int':
            case 'boolean':
            case 'date':
    ...
    static getFieldDef(field, fieldType, {
        notNull = true,
        defaultValue = null,
        collate = '',
        onConflict = '',
        ...
        const notNullStmt = notNull || pk ? ' NOT NULL' : '';
        const sqlType = Db.getSqlType(fieldType);
        const defaultStmt = defaultValue === null || pk ? '' : ` DEFAULT ${this.quote(defaultValue)}`;
        collate = collate.trim().toUpperCase();
        const collateStmt = collate.length && sqlType === 'TEXT' ? ` COLLATE ${collate}` : '';
        onConflict = onConflict.trim().toUpperCase();
        const onConflictStmt = onConflict.length ? ` ON CONFLICT ${onConflict}` : '';
        const pkStmt = pk ? ' PRIMARY KEY' : '';
        const autoIncrementStmt = pk && sqlType === 'INTEGER' && autoIncrement ? ' AUTOINCREMENT' : '';
        return `"${field}" ${sqlType}${notNullStmt}${defaultStmt}${collateStmt}${onConflictStmt}${pkStmt}${autoIncrementStmt}`;
    }

    static getPkFieldDef(field, fieldType, {
        collate = '',
        onConflict = '',
        autoIncrement = true,
    } = {}) {
        return Db.getFieldDef(field, fieldType, {
            defaultValue: null,
            notNull: true,
            collate,
            onConflict,
    ...
    static getTableDef(table, fieldTypes, fieldProps = {}, {
        ifNotExists = true,
    } = {}) {
        const fieldStmts = [];
        for (const field in fieldTypes) {
            fieldStmts.push(this.getFieldDef(field, fieldTypes[field], fieldProps[field] ?? {}))
        }
        const ifNotExistsStmt = ifNotExists ? ' IF NOT EXISTS' : '';
        return `CREATE TABLE${ifNotExistsStmt} "${table}" (\n${fieldStmts.map(item => '  ' + item).join(',\n')}\n);`;
    }
                            ...
                            if (resultField in fieldTypes) {
                                try {
                                    for (const row of rows) {
                                        set.add(Db.getFieldValue(row[resultField], fieldTypes[resultField]));
                                    }
                                } catch (valueError) {
                                    reject(valueError);
                                }
                        ...
                        case 'map':
                            ...
                            if (resultField.length) {
                                if (resultField in fieldTypes) {
                                    try {
                                        for (const row of rows) {
                                            map.set(row[resultKey], Db.getFieldValue(row[resultField], fieldTypes[resultField]));
                                        }
                                    } catch (valueError) {
                                        reject(valueError);
                                    }
                        ...
                        case 'object':
                            ...
                            if (resultField.length) {
                                if (resultField in fieldTypes) {
                                    try {
                                        for (const row of rows) {
                                            object[row[resultKey]] = Db.getFieldValue(row[resultField], fieldTypes[resultField]);
                                        }
                                    } catch (valueError) {
                                        reject(valueError);
                                    }
                        ...
                        default:
                            ...
                            if (resultField.length) {
                                if (resultField in fieldTypes) {
                                    try {
                                        for (const row of rows) {
                                            arr.push(Db.getFieldValue(row[resultField], fieldTypes[resultField]));
                                        }
                                    } catch (valueError) {
                                        reject(valueError);
                                    }
    ...
    async get(sql, {params = [], resultField = '', fieldTypes = {}} = {}) {
        return new Promise((resolve, reject) => {
            this.db.get(sql, params, (err, row) => {
                ...
                try {
                    resolve(Db.getFieldValue(value, fieldTypes[resultField]));
                } catch (valueError) {
                    reject(valueError);
                }
            });
